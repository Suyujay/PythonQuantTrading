...
WRITER_PATH = utils_folder_path + "/Chapter4/4-3/writer/attention/"
# 設置模型參數保存的路徑
MODELPARAM_PATH = utils_folder_path + "/Chapter4/4-3/model_param/attention/"
...
# %%
""" 模型定義 """
# 定義一個基於多頭注意力機制的模型，用於時間序列預測。


class Model(nn.Module):
    def __init__(
        self, input_size, attn_embed_dim, attn_num_heads, dropout, output_size
    ):
        super(Model, self).__init__()
        # 定義一個線性層，將輸入特徵的維度變換到注意力機制的嵌入維度
        self.linear1 = nn.Linear(in_features=input_size, out_features=attn_embed_dim)
        # 定義多頭注意力機制
        ...
        # 定義另一個線性層，將注意力機制的輸出映射到最終輸出的維度
        self.linear2 = nn.Linear(in_features=attn_embed_dim, out_features=output_size)

    def forward(self, inputs):
        # 透過第一個線性層將輸入映射到注意力機制所需的嵌入維度
        # (batch_size, seq_length, features_nums) -> (batch_size, seq_length, attn_embed_dim)
        outputs = self.linear1(inputs)

        # 透過多頭注意力機制進行處理
        # self.self_attn 的輸入為 (query, key, value)，此處 query, key 和 value 都使用線性層的 outputs
        # (batch_size, seq_length, attn_embed_dim) -> (batch_size, seq_length, attn_embed_dim)
        outputs, _ = self.self_attn(outputs, outputs, outputs)

        # (batch_size, seq_length, attn_embed_dim) -> (batch_size, attn_embed_dim)
        outputs = outputs[:, -1, :]

        # (batch_size, attn_embed_dim) -> (batch_size, output_size)
